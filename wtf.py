...
train_df = pd.read_csv('different processed dataframes/normal_processed.tsv', '\t')
test_df = pd.read_csv('normal_processed_test.tsv', '\t')

train_data = train_df['clean'].values.astype('str')
# train_data = train_df['nps'].values
y_train_misog = train_df['misogynous']
y_train_aggrs = train_df['aggressiveness']

test_data = test_df['clean'].values.astype('str')
...
embeddings_index = {}
with open('glove/glove.twitter.27B.200d_small.txt', 'r', encoding='utf-8') as f:
    for line in f:
        values = line.split()
        word = values[0]
...
X,X_test=get_mean_embeddings(train_data,embeddings_index),get_mean_embeddings(test_data,embeddings_index)
